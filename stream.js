 ...
 * Streaming thread
 */
var startStream = function(hashtags) {
...
var updateHashtags = function(hashtags) {
    Search.find().sort('-created').exec(function(err, searches) {
        if (err) {
            ...
            var newHashtags = [];
            for (i in searches) {
                if (searches[i].hashtags.length) {
                    newHashtags.push('#' + searches[i].hashtags);
                }
            }
            console.log(newHashtags.join(','));
            if (newHashtags != hashtags) {
                hashtags = newHashtags;
                startStream(hashtags);
            }
        }
    });
}
